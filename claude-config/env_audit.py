...
VARS = [
    ...
    ("ANTHROPIC_API_KEY", "auth", "Console API key, sent as X-Api-Key. Pay-per-token. Beats a subscription login in -p mode"),
    ("ANTHROPIC_AUTH_TOKEN", "auth", "Raw Bearer token for the Authorization header (gateways, OAuth)"),
    ("CLAUDE_CODE_OAUTH_TOKEN", "auth", "Long-lived token from `claude setup-token` (Pro/Max subscription) for CI"),
    ("ANTHROPIC_CUSTOM_HEADERS", "auth", "Extra headers, 'Name: Value' per line (e.g. Bedrock guardrails)"),
    # --- model routing ---
    ("ANTHROPIC_MODEL", "model", "Model for this session. Overrides `model` in every settings file; --model overrides this"),
    ("ANTHROPIC_DEFAULT_MODEL", "model", "Model for NEW sessions when nothing else picks one"),
    ("ANTHROPIC_DEFAULT_OPUS_MODEL", "model", "What the `opus` alias resolves to (pin versions on Bedrock/Vertex)"),
    ("ANTHROPIC_DEFAULT_SONNET_MODEL", "model", "What the `sonnet` alias resolves to"),
    ("ANTHROPIC_DEFAULT_HAIKU_MODEL", "model", "What `haiku` resolves to; also used for background tasks (titles, summaries)"),
    ("ANTHROPIC_SMALL_FAST_MODEL", "model", "[deprecated] old name for the Haiku-class background model"),
    ("MAX_THINKING_TOKENS", "model", "Cap thinking budget on fixed-budget models (adaptive models use /effort instead)"),
    # --- provider switches ---
    ("CLAUDE_CODE_USE_BEDROCK", "provider", "=1 routes model traffic to Amazon Bedrock (AWS creds via default chain)"),
    ...
    ("CLAUDE_CODE_USE_VERTEX", "provider", "=1 routes to Google Cloud Agent Platform (Vertex); auth via gcloud ADC"),
    ...
    ("ANTHROPIC_BASE_URL", "network", "Point the Anthropic API at an LLM gateway (LiteLLM etc.) instead of api.anthropic.com"),
    # --- network ---
    ("HTTPS_PROXY", "network", "Corporate proxy for outbound HTTPS (https_proxy lowercase also honoured; no SOCKS)"),
    ...
    ("NODE_EXTRA_CA_CERTS", "network", "PEM bundle of extra CAs — the fix for TLS-inspecting proxies when the root isn't in the OS store"),
    ("CLAUDE_CODE_CERT_STORE", "network", "Which trust stores to use: bundled,system (default) | bundled | system"),
    ...
    ("CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC", "flags", "One switch for telemetry + error reports + changelog fetch + other non-model traffic"),
    ("BASH_DEFAULT_TIMEOUT_MS", "flags", "Default Bash tool timeout (120000). This repo sets 300000 in .claude/settings.json"),
    ...
    ("ENABLE_CLAUDEAI_MCP_SERVERS", "flags", "=false stops fetching claude.ai connectors via mcp-proxy.anthropic.com"),
    ("CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", "flags", "=1 enables agent teams (~7x tokens)"),
    # --- location ---
    ("CLAUDE_CONFIG_DIR", "config", "Move ~/.claude somewhere else (settings, sessions, credentials all follow)"),
    # --- set BY Claude Code in subprocesses (read-only signals) ---
    ("CLAUDECODE", "signal", "=1 inside any process Claude Code spawned — scripts can detect they're under Claude"),
...
def table() -> None:
    ...
    print("-" * 140)
    print(f"{n_set} of {len(VARS)} set. Precedence for auth: ANTHROPIC_API_KEY > ANTHROPIC_AUTH_TOKEN > subscription login.")
    print("Set project-wide ones in .claude/settings.json `env`; personal ones in ~/.claude/settings.json `env`;")
...
def live_api() -> None:
    ...
    print(f"[live] usage         = in {r.usage.input_tokens}, out {r.usage.output_tokens}, "
